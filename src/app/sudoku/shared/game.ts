...
export class SudokuGame {
  normalizeInput(value: unknown): number {
  ...
  isSellEditable(board: Board, cell: CellPosition): boolean {
    return board[cell.y][cell.x] === 0;
  }

  upsertSelector(selectors: Selector[], selector: Selector): Selector[] {
  ...
  upsertInput(inputs: CellInput[], input: CellInput): CellInput[] {
  ...
  upsertUser(users: User[], user: User): User[] {
  ...
  leaveUser(users: User[], user: User): User[] {
